...
const char* HEADER_V00_01 = "ESPLAY_FIRMWARE_V00_01";
...
void flash_firmware(const char* fullPath)
{
    ...
    while (true)
    {
        ...
        if(!previousState.values[GAMEPAD_INPUT_START] && state.values[GAMEPAD_INPUT_START])
        {
            break;
        }
    ...
    int parts_count = 0;
    esplay_partition_t* parts = malloc(sizeof(esplay_partition_t) * PARTS_MAX);
    if (!parts)
    {
...
const char* ui_choose_file(const char* path)
{
    ...
    files = 0;
    fileCount = sdcard_files_get(path, ".fw", &files);
